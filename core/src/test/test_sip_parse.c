...
void test_sip_header_get(void) {
  ...
  ASSERT("Call-ID value correct",
         val_len == strlen("testcall@10.0.0.2") && strncmp(val, "testcall@10.0.0.2", val_len) == 0);
}
...
void test_sip_prepend_via(void) {
  char out[2048];
  int  len =
      sip_prepend_via(INVITE_REQ, strlen(INVITE_REQ), "SIP/2.0/UDP 10.0.0.9:5060;branch=z9hG4bK-new", out, sizeof(out));
...
void test_sip_strip_top_via(void) {
  ...
  int  len1 = sip_prepend_via(INVITE_REQ, strlen(INVITE_REQ), "SIP/2.0/UDP 10.0.0.9:5060;branch=z9hG4bK-new", with_via,
                              sizeof(with_via));
