...
typedef struct sip_tcp_connection {
  ...
  time_t created;        /* Time when connection was created */
...
static void apply_trunk_rewrites(config_trunk *trunk, const char *input, char *output, size_t out_size) {
  ...
  if (!trunk || !trunk->rewrite_regex || trunk->rewrite_count == 0 || !output || out_size == 0) {
    if (output && out_size) {
      snprintf(output, out_size, "%s", input ? input : "");
    }
    return;
  }
...
static void handle_register(const char *req_buf, size_t req_len, upbx_config *cfg, sip_send_ctx *ctx) {
  ...
  /* Get source address for security tracking */
  char src_host[64] = {0};
  char src_port[16] = {0};
  struct sockaddr_in *sin = (struct sockaddr_in *)&ctx->peer;
  if (ctx->peerlen >= sizeof(struct sockaddr_in) && ctx->peer.ss_family == AF_INET) {
    inet_ntop(AF_INET, &sin->sin_addr, src_host, sizeof(src_host));
    snprintf(src_port, sizeof(src_port), "%d", ntohs(sin->sin_port));
  }

  char *contact_val = get_contact_value(req_buf, req_len);
  registration_update(extension_num, raw_uri_user, trunk_name_str, contact_val, h, p[0] ? p : "5060", src_host, src_port, NULL, ctx->is_tcp ? "tcp" : "udp");
  /* Store TCP socket on registration if this is a TCP connection.
   * Use NULL trunk_name to find any registration with this number (handles transport switch). */
...
static int daemon_tcp_sockfd = -1;

static char overflow_wrapper(struct pt *pt, int64_t timestamp, struct pt_task *task) {
  (void)task;
  return overflow_pt(pt, global_cfg, daemon_sockfd);
}
...
static int send_fork_invite_to_ext(call_t *call, upbx_config *cfg,
  ...
  /* Determine transport to use when sending INVITE to this extension:
   * ALWAYS use extension's registered transport - never use any transport from the original call.
   * Default to UDP if extension registered with UDP or unknown. */
  int use_tcp = 0;
  if (ext->transport[0] && strcasecmp(ext->transport, "tcp") == 0) {
    /* Extension registered with TCP */
    use_tcp = 1;
  }
  if (use_tcp) {
   ...
   * This ensures trunk/extension connects to our rtpproxy, not to some learned address. */
  {
    ...
    if (call->rtp_port_callee > 0 &&
      ...
      char new_sdp[4096];
      /* Force callee to send first RTP to punch NAT */
      int sdp_len = sdp_rewrite_addr_with_transport(cur_body, cur_body_len, sdp_host, call->rtp_port_callee, use_tcp, 1, new_sdp, sizeof(new_sdp));
      if (sdp_len > 0) {
...
static void fork_invite_to_extension_regs(upbx_config *cfg, const char *req_buf, size_t req_len, sip_send_ctx *ctx,
  ...
  /* Set transport for caller side (side A) - from trunk config */
  if (trunk) {
    call_set_transport(call, 1, trunk->transport[0] ? trunk->transport : "udp");
  } else {
    call_set_transport(call, 1, ctx->is_tcp ? "tcp" : "udp");
  }
  ...
  if (sdp_n > 0 && sdp_media[0].port > 0) {
    ...
    if (call->trunk_name && call->trunk_name[0]) {
      ...
      call->ext_sdp_is_tcp = sdp_media[0].is_tcp;    /* extension's SDP */
    }
  }

  /* Create rtpproxy sessions: one for caller side, one for callee side. */
  /* ALL CALLS MUST GO THROUGH RTPPROXY - never allow direct phone-to-phone */
  ...
  if (call_rtpproxy_session_create(call, 1, caller_tag, callee_tag) != 0) {
    log_error("RTP: failed to create session for caller side, call %.32s - REJECTING", call_id_buf);
    rtp_failed = 1;
  }
  if (call_rtpproxy_session_create(call, 0, callee_tag, caller_tag) != 0) {
    log_error("RTP: failed to create session for callee side, call %.32s - REJECTING", call_id_buf);
    rtp_failed = 1;
  }
  if (rtp_failed) {
    call_remove(call);
...
static void handle_invite_outgoing(upbx_config *cfg, const char *buf, size_t len, sip_send_ctx *ctx, const char *from_user,
  ...
  memset(&hints, 0, sizeof(hints));
  hints.ai_family = AF_UNSPEC;
  /* Use trunk's configured transport */
  int trunk_tcp = (trunk->transport[0] && strcasecmp(trunk->transport, "tcp") == 0);
  hints.ai_socktype = trunk_tcp ? SOCK_STREAM : SOCK_DGRAM;
  ...
  if (call) {
    ...
    /* Set transport for caller side (side A) - from their registration */
    {
      const char *caller_transport = registration_get_transport(reg);
    ...
    if (get_advertise_addr(cfg, via_host, sizeof(via_host), via_port_s, sizeof(via_port_s)) == 0) {
      ...
      if (cur_len > 0 && call->rtp_port_caller > 0 && sdp_n > 0) {
        ...
        if (sip_request_get_body(t1, (size_t)cur_len, &sb, &sbl) && sbl > 0) {
          ...
          sdp_host[sizeof(sdp_host) - 1] = '\0';
          char new_sdp[4096];
          /* PBX is trunk's bitch - use trunk's SDP transport, not extension's transport */
          int use_trunk_tcp = call->trunk_sdp_is_tcp ? 1 : (trunk_tcp ? 1 : 0);
          int sdp_len = sdp_rewrite_addr_with_transport(sb, sbl, sdp_host, call->rtp_port_caller, use_trunk_tcp, 0, new_sdp, sizeof(new_sdp));
          if (sdp_len > 0) {
...
static void handle_invite(upbx_config *cfg, const char *buf, size_t len, sip_send_ctx *ctx) {
  ...
  /* 1. From a known extension - verify CALLER is registered (security) */
  ...
  if (from_ext_configured) {
    /* Security: verify caller has valid registration that matches source (IP for UDP, socket for TCP) */
    int is_tcp = ctx->is_tcp;
    int sockfd = ctx->sockfd;
    
    /* For TCP, verify the connection is registered */
    if (is_tcp) {
      ext_reg_t *reg_by_sock = registration_get_by_tcp_sock(sockfd);
      log_debug("TCP security: sockfd=%d reg_by_sock=%p number=%s from_ext=%s", 
                sockfd, reg_by_sock, reg_by_sock ? reg_by_sock->number : "(null)", from_ext);
      if (!reg_by_sock) {
        log_warn("INVITE rejected: TCP socket %d has no registration, challenging with 401", sockfd);
        send_reply(ctx, buf, len, 401, 0, 0);
        return;
      }
      if (strcmp(reg_by_sock->number, from_ext) != 0) {
        log_warn("INVITE rejected: TCP socket %d registered as %s but call from %s", 
                 sockfd, reg_by_sock->number, from_ext);
        send_reply(ctx, buf, len, 403, 0, 0);
        return;
      }
    } else {
      /* For UDP, verify source IP:port matches remote_addr where we received REGISTER from */
      ext_reg_t **caller_regs = NULL;
      size_t n_caller_regs = registration_get_regs(NULL, from_ext, &caller_regs);
      if (n_caller_regs == 0 || !caller_regs) {
        log_warn("INVITE rejected: caller %s not registered", from_ext);
        send_reply(ctx, buf, len, 403, 0, 0);
        return;
      }
      struct sockaddr_in *sin = (struct sockaddr_in *)&ctx->peer;
      int ip_port_matched = 0;
      for (size_t i = 0; i < n_caller_regs && !ip_port_matched; i++) {
        if (caller_regs[i]->remote_addr.sin_family == AF_INET &&
            caller_regs[i]->remote_addr.sin_addr.s_addr == sin->sin_addr.s_addr &&
            caller_regs[i]->remote_addr.sin_port == sin->sin_port) {
          ip_port_matched = 1;
        }
      }
      free(caller_regs);
      if (!ip_port_matched) {
        ...
        log_warn("INVITE rejected: caller %s not registered from %s", from_ext, src_host);
        send_reply(ctx, buf, len, 403, 0, 0);
        return;
      }
    }
...
static void handle_sip_response(upbx_config *cfg, const char *buf, size_t len, sip_send_ctx *ctx) {
  ...
  if (code >= 200 && code < 300) {
    ...
    if (sip_request_get_body(tmp1, (size_t)cur_len, &resp_body, &resp_body_len) && resp_body_len > 0) {
      ...
      if (call->rtp_port_caller > 0 &&
        ...
        /* Force caller to send first RTP to punch NAT, then receive from rtpproxy */
        int sdp_len = sdp_rewrite_addr_with_transport(resp_body, resp_body_len, sdp_host, call->rtp_port_caller, use_tcp, 1, new_sdp, sizeof(new_sdp));
        if (sdp_len > 0) {
  ...
  /* Handle non-2xx responses: update fork state and decide forward behavior */
  if (code >= 300 && fork_idx != (size_t)-1) {
    /* Send ACK to callee for non-2xx responses (RFC 3261) */
    char ack_cseq[64];
    int ack_cseq_num = get_cseq_number(buf, len);
    snprintf(ack_cseq, sizeof(ack_cseq), "%d ACK", ack_cseq_num);
    char ack_req_uri[256] = {0};
    fork_uri_from_addr(&call->fork_addrs[fork_idx], call->fork_ids[fork_idx], ack_req_uri, sizeof(ack_req_uri));
    if (ack_req_uri[0]) {
      char ack_via[512] = {0}, ack_from[384] = {0}, ack_to[384] = {0}, ack_callid[256] = {0};
      ...
      char *ack = sip_build_request_parts("ACK", ack_req_uri, ack_via, ack_from, ack_to, ack_callid, ack_cseq, NULL, 0, NULL, 0, NULL);
      if (ack) {
        log_trace("SIP: ACK to callee for %d", code);
        udp_send_to(ctx->sockfd, (struct sockaddr *)&ctx->peer, ctx->peerlen, ack, strlen(ack));
        free(ack);
      }
    }

    /* Update fork state and decide forward behavior */
    if (code == 486 && !call->cancelling) {
      ...
      log_debug("SIP response: fork %zu busy, added to pending", fork_idx);
      should_forward = 0;
    } else {
      call->fork_state[fork_idx] = FORK_DONE;
    }

    /* Check if all forks done */
    int all_done = 1;
    for (size_t f = 0; f < call->n_forks; f++) {
    ...
    if (all_done && call->n_pending_exts == 0) {
      ...
      if (call->fork_state[fork_idx] != FORK_ACTIVE) should_forward = 0;
    }
  }
...
static void handle_udp_msg(upbx_config *cfg, udp_msg_t *msg, int sockfd) {
  ...
  if (method_len == 8 && strncasecmp(method, "REGISTER", 8) == 0) {
    ...
    /* Security: verify source is party A or B */
    if (c) {
      int from_a = sockaddr_match(&ctx.peer, ctx.peerlen, (struct sockaddr_storage *)&c->a.sip_addr, c->a.sip_len);
      int from_b = sockaddr_match(&ctx.peer, ctx.peerlen, (struct sockaddr_storage *)&c->b.sip_addr, c->b.sip_len);
      if (!from_a && !from_b) {
        log_warn("ACK rejected: source not party in call %.32s", cid);
        send_reply(&ctx, buf, len, 403, 0, 0);
        return;
      }
    }
    ...
    if (c) {
      ...
      int from_b = sockaddr_match(&ctx.peer, ctx.peerlen, (struct sockaddr_storage *)&c->b.sip_addr, c->b.sip_len);
      if (!from_a && !from_b) {
        log_warn("BYE rejected: source not party in call %.32s", cid);
        send_reply(&ctx, buf, len, 403, 0, 0);
        return;
      }
      /* Determine who sent the BYE and forward to the other side. */
      struct sockaddr_storage *fwd_addr = NULL;
      socklen_t fwd_len = 0;
    ...
    if (c) {
      ...
      int from_b = sockaddr_match(&ctx.peer, ctx.peerlen, (struct sockaddr_storage *)&c->b.sip_addr, c->b.sip_len);
      if (!from_a && !from_b) {
        log_warn("CANCEL rejected: source not party in call %.32s", cid);
        send_reply(&ctx, buf, len, 403, 0, 0);
        return;
      }
    }
...
/* Create and bind TCP listening socket; return fd or -1. */
static int tcp_bind(const char *listen_addr) {
  char host[256], port[32];
  struct addrinfo hints, *res = NULL, *ai;
  int fd = -1;
  int ret;
...
PT_THREAD(sip_tcp_client_pt(struct pt *pt, int64_t timestamp, struct pt_task *task)) {
  ...
  #define TCP_UNREGISTERED_TIMEOUT 10

  for (;;) {
    int ready_fd = -1;
    time_t now = time(NULL);
    ext_reg_t *reg = registration_get_by_tcp_sock(state->fd);

    /* Close connection if open >10s without registration */
    int age = (int)(now - state->created);
    if (!reg && state->fd >= 0 && age > TCP_UNREGISTERED_TIMEOUT) {
      log_debug("TCP: closing unregistered connection %s (age=%d > %ds), fd=%d", 
                state->peer_str, age, TCP_UNREGISTERED_TIMEOUT, state->fd);
      close(state->fd);
      state->fd = -1;
      free(state);
      PT_EXIT(pt);
...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
    PT_WAIT_UNTIL(pt,
      ...
      (!reg && (now - state->created) > TCP_UNREGISTERED_TIMEOUT)));
#pragma GCC diagnostic pop

    /* After waking up, re-check registration in case it was registered while we were waiting */
    if (state->fd < 0) {
      free(state);
      PT_EXIT(pt);
    }
    reg = registration_get_by_tcp_sock(state->fd);
    if (!reg && state->fd >= 0 && time(NULL) - state->created > TCP_UNREGISTERED_TIMEOUT) {
      log_debug("TCP: closing unregistered connection %s after %ds (post-wait)", 
                state->peer_str, TCP_UNREGISTERED_TIMEOUT);
      close(state->fd);
      state->fd = -1;
      free(state);
      PT_EXIT(pt);
            ...
            if (sip_request_method(buf, (size_t)n, &method, &method_len)) {
              if (method_len == 8 && strncasecmp(method, "REGISTER", 8) == 0) {
                ...
                if (c) {
                  ...
                  if (!from_a && !from_b) {
                    log_warn("ACK rejected: source not party in call %.32s", cid);
                    send_reply(&ctx, buf, (size_t)n, 403, 0, 0);
                  } else if (c->answered && c->b.sip_len > 0) {
                    char via_host[256], via_port[32];
...
PT_THREAD(daemon_root_pt(struct pt *pt, int64_t timestamp, struct pt_task *task)) {
  ...
  for (;;) {
    ...
    if (n > 0) {
      if (FD_ISSET(daemon_sockfd, &r)) {
        peerlen = sizeof(peer);
        ssize_t nr = recvfrom(daemon_sockfd, buf, sizeof(buf), 0, (struct sockaddr *)&peer, &peerlen);
        if (nr > 0 && (size_t)nr <= SIP_READ_BUF_SIZE - 1) {
          ...
          if (msg) {
            ...
            msg->buf[nr] = '\0';
            handle_udp_msg(global_cfg, msg, daemon_sockfd);
          }
