...
pbx_registration_t *pbx_route(const char *source_extension, const char *dialed_number, const char *source_group_prefix) {
  ...
  if (source_group_prefix) {
    ...
    if (reg) {
      ...
      if (!is_cross_group(src_group, dst_group) || 
          (src_ext && src_group && pbx_group_find(src_group)->allow_outgoing_cross_group) ||
          !src_ext) {
        exact_group_num = reg;
      }
    }
  ...
  log_trace("pbx:   exact_group_num=%s, exact_num=%s", 
            exact_group_num ? exact_group_num->extension : "none",
