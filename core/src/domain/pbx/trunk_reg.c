...
static int build_register(trunk_reg_t *t, char *buf, size_t buf_size, const char *realm, const char *nonce) {
  ...
  int len = snprintf(buf, buf_size,
                     ...
                     "Content-Length: 0\r\n"
                     "\r\n",
                     t->host, t->port, t->host, t->port, t->branch, t->username ? t->username : "anonymous", t->host,
                     (unsigned long long)(now ^ 0x12345678), t->username ? t->username : "anonymous", t->host,
                     t->call_id, t->cseq, t->username ? t->username : "anonymous", t->host, t->port,
                     TRUNK_REG_DEFAULT_EXPIRY, UPBX_VERSION_STR, auth_header[0] ? auth_header : "",
                     auth_header[0] ? "\r\n" : "", auth_header[0] ? "" : "");
...
static int send_register(trunk_reg_t *t, const char *realm, const char *nonce) {
  ...
  ssize_t sent = sendto(t->fd, buf, (size_t)len, 0, (struct sockaddr *)&t->remote_addr, t->remote_addr_len);
  if (sent != len) {
...
static int parse_www_authenticate(const char *value, size_t value_len, char *realm, size_t realm_size, char *nonce,
                                  size_t nonce_size) {
...
static void handle_response(trunk_reg_t *t, int64_t timestamp) {
  ...
  if (msg.status_code == 200) {
    ...
    if (auth && auth_len > 0) {
      ...
      if (parse_www_authenticate(auth, auth_len, realm, sizeof(realm), nonce, sizeof(nonce)) == 0) {
...
int trunk_reg_pt(int64_t now, struct pt_task *task) {
  ...
  if (t->state == 0) {
    ...
    int gai_ret = getaddrinfo(t->host, port_str, &hints, &res);
    if (gai_ret != 0 || !res) {
  ...
  if (ready_fd == t->fd) {
    ...
    t->recv_len =
        recvfrom(t->fd, t->recv_buf, sizeof(t->recv_buf) - 1, 0, (struct sockaddr *)&t->from_addr, &t->from_len);
    if (t->recv_len > 0) {
...
void trunk_reg_start_all(void) {
  ...
  resp_object *cfg = domain_cfg;
  if (cfg->type != RESPT_ARRAY) {
    log_warn("trunk_reg: config is not an array");
    return;
  }

  for (size_t i = 0; i + 1 < cfg->u.arr.n; i += 2) {
    if (cfg->u.arr.elem[i].type != RESPT_BULK) continue;
    const char *key = cfg->u.arr.elem[i].u.s;
    if (!key || strncmp(key, "trunk:", 6) != 0) continue;

    if (cfg->u.arr.elem[i + 1].type != RESPT_ARRAY) continue;
    resp_object *sec = &cfg->u.arr.elem[i + 1];
    ...
    sched_create(trunk_reg_pt, t);
    log_info("trunk_reg: started protothread for trunk '%s' -> %s:%d", t->name, t->host, t->port);
  }
