...
/* Start plugins. discovery_cmd: sent to discover all methods the plugin supports (e.g. "command"). Discovery skipped if
 * NULL. after_discovery_cb: optional; called after each plugin's discovery (and when config is resent without restart).
 */
void plugmod_start(const plugmod_config_item *configs, size_t n, const char *discovery_cmd,
...
/* Start one plugin (spawn, discovery). discovery_cmd must be set (e.g. by prior plugmod_start or plugmod_sync). Returns
 * 0 on success. */
int plugmod_start_plugin(const char *name, const char *exec, unsigned long long config_hash, int restart_on_update);

/* Sync plugins to config: tick, then stop removed/changed, start added. Stores discovery_cmd and after_discovery
 * callback. */
void plugmod_sync(const plugmod_config_item *configs, size_t n, const char *discovery_cmd,
                  plugmod_after_discovery_fn after_discovery_cb, void *after_discovery_user);
