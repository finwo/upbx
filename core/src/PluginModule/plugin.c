...
void plugmod_start(const plugmod_config_item *configs, size_t n, const char *discovery_cmd,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (spawn_plugin(configs[i].name, configs[i].exec, p) != 0) continue;
    ...
    if (after_discovery_cb_used) after_discovery_cb_used(p->name, after_discovery_user_used);
...
static void remove_plugin_at(size_t i) {
  ...
  if (i < plugin_n - 1) memmove(&plugins[i], &plugins[i + 1], (plugin_n - 1 - i) * sizeof(plugin_state_t));
  plugin_n--;
}
...
void plugmod_sync(const plugmod_config_item *configs, size_t n, const char *discovery_cmd,
  ...
  for (size_t i = 0; i < plugin_n; i++) {
    ...
    for (size_t j = 0; j < n; j++)
      if (configs[j].name && strcmp(p->name, configs[j].name) == 0) {
        c = &configs[j];
        break;
      }
    if (!c) {
...
void plugmod_stop(void) {
  for (size_t i = 0; i < plugin_n; i++) {
    if (plugins[i].pid > 0) kill(plugins[i].pid, SIGTERM);
    plugin_state_free(&plugins[i]);
  }
  plugin_n = 0;
  free(plugins);
...
int plugmod_has_method(const char *plugin_name, const char *method_name) {
  ...
  for (size_t i = 0; i < p->method_count; i++)
    if (strcasecmp(p->methods[i], method_name) == 0) return 1;
  return 0;
}
